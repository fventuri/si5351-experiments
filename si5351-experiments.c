...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (feedback_ms < 15 || feedback_ms > 90) {
            fprintf(stdout, "invalid feedback MS: %'.0lf (xtal=%'.0lf)\n", feedback_ms, xtal);
            fprintf(stdout, "\n");
            output_ms -= 2;
            continue;
        }
        ...
        fprintf(stdout, "actual PLL frequency: %'.0lf * (%d + %d / %d) = %'.0lf%s\n", xtal, a, b, c, actual_pll_freq, is_integer);
    ...
    if (feedback_ms < 16) {
        fprintf(stderr, "invalid feedback MS: %d (xtal=%'.0lf)\n", feedback_ms, xtal);
        return EXIT_FAILURE;
    }
    if (feedback_ms > 90) {
        feedback_ms = 90;
    ...
    while (1) {
        ...
        fprintf(stdout, "actual PLL frequency: %'.0lf * %d\n", xtal, feedback_ms);
